...
export class OrdersController {
  ...
  findAll(@Query() query: OrderPaginationDto) {
    return this.ordersClient.send('findAllOrders', query).pipe(
      catchError((error) => {
        throw new RpcException(error);
      })
    );
  ...
  findOne(@Param('uuid', ParseUUIDPipe) uuid: string) {
    return this.ordersClient.send('findOneOrder', { uuid }).pipe(
      catchError((error) => {
        throw new RpcException(error);
      })
    );
  ...
  changeStatus(@Param('uuid', ParseUUIDPipe) uuid: string, @Body() updateOrderDto: UpdateOrderDto) {
    return this.ordersClient.send('changeOrderStatus', { uuid, ...updateOrderDto }).pipe(
      catchError((error) => {
        throw new RpcException(error);
      })
    );
